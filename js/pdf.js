...
window.handleFileUpload = async function(event) {
    ...
    if (file && file.name.toLowerCase().endsWith('.pdf')) {
        ...
        if (window.switchLeftView) {
            window.switchLeftView('pdf-view', document.querySelectorAll('.topbar-right .main-tabs .tab-item')[0]);
        }
        ...
        try {
            ...
            if (fullText.trim().length < 100) {
                // OCR Fallback
                accordion.innerHTML = '<div style="padding: 20px; color: var(--accent-active); text-align: center;">Scanned PDF detected. Running OCR (Optical Character Recognition)... this may take a minute.</div>';
                ...
                for (let i = startPage; i <= endPage; i++) {
                    pagesScanned++;
                    accordion.innerHTML = `<div style="padding: 20px; color: var(--accent-active); text-align: center;">Scanning page ${i} (Progress: ${pagesScanned} of ${totalPagesToScan}) using OCR...</div>`;
                    const page = await pdf.getPage(i);
            ...
            accordion.innerHTML = '<div style="padding: 20px; color: red; text-align: center;">Error extracting PDF text. Please ensure the file is not corrupted or password protected.</div>';
        }
    } else {
            ...
            <div class="accordion-content">
                ...
                <div class="next-step-block">
                    ...
                    <div class="next-step-body">
                        ...
                        <div style="display:flex; gap:10px;">
                            ...
                            <button class="btn-dark-pill" onclick="window.switchLeftView('pdf-view', document.querySelectorAll('.topbar-right .main-tabs .tab-item')[0])">📄 Read PDF</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="action-cards-grid">
                    <div class="action-card card-purple">
                        <span style="font-weight:bold; display:flex; align-items:center; gap:8px;">🃏 Flashcards</span>
                        ${sec.flashcardsGenerated ? `<button class="btn-action" style="background:#22c55e; color:white; border-color:#16a34a; cursor:default;" disabled>✅ Cards Generated</button>` : `<button class="btn-action" onclick="generateCards('${sec.title.replace(/'/g, "\\'")}')">Generate Cards</button>`}
                    </div>
                    <div class="action-card card-green">
                        <span style="font-weight:bold; display:flex; align-items:center; gap:8px;">❓ Quiz</span>
